...
class DriverHomePage extends Component {
	...
	handleDriverGoOnline = () => {
		...
		const driver = this.props.loggedInUser
			...
			features: [
				{
					...
					properties: {
						...
						description:
							'<h3><strong>Pickup Location</strong></h3><p><a href="http://www.mtpleasantdc.com/makeitmtpleasant" target="_blank" title="Opens in a new window">Make it Mount Pleasant</a',
					},
				},
			],
		}
	...
	handleDriveToUser = e => {
		...
		const requestDetails = JSON.parse(
			localStorage.getItem('requestDetails')
		...
		document
			.querySelectorAll('.driver-map')[0]
			.classList.remove('hide-direction')

		setTimeout(() => {
			const direcitonInstruction = document.querySelectorAll(
				'.directions-control .mapbox-directions-instructions'
			)[0]
			if (direcitonInstruction) {
				direcitonInstruction.style.display = 'none'
			}
			let summaryHeader = document.querySelectorAll(
			...
			btn.addEventListener('click', function() {
				let dirInstruction = document.querySelectorAll(
	...
	onTripRequestCanceledByRider = () => {
		...
		console.log('YonTripRequestCanceledByRider')
	...
	onTripCanceledByRider = () => {
		this.setState({
			driverStatus: 'standby',
			headerMessage:
				'The trip has been cancelled. Finding another ride for you',
			requestDetails: null,
		}) //Save request details

		console.log('TRIP_CANCELED_BY_RIDER! \n')
		helper.removeTrip()
		document
			.querySelectorAll('.driver-map')[0]
			.classList.add('hide-direction')
	...
	onTripCanceledByDriver = data => {
		console.log('TRIP_CANCELED_BY_DRIVER! \n')
		this.setState({
			driverStatus: 'standby',
			requestDetails: null,
		})
	...
	componentDidMount() {
		...
		navigator.geolocation.getCurrentPosition(position => {
				...
				features: [
					{
						...
						properties: {
							title: 'Your are here',
							description:
								'<h3><strong>You are here</strong></h3><p>2398 Walters way</p>',
						},
					},
				],
			}
			...
			this.updateDriverLocation([position.coords.longitude, position.coords.latitude])
			...
			this.map.on('load', () => {
				this.startInput = document.querySelectorAll(
					'.mapbox-directions-origin input'
				)[0]
				this.destInput = document.querySelectorAll(
					'.mapbox-directions-destination input'
				)[0]
				console.log(
					'document.querySelectorAll(".mapbox-directions-instructions")',
					document.querySelectorAll('.mapbox-directions-instructions')
				)
			})
	...
	bindListeners() {
		...
		socket.on(
			'TRIP_REQUEST_CANCELED_BY_RIDER',
			this.onTripRequestCanceledByRider
		)
	...
	render() {
		...
		requestDetails =
			JSON.parse(localStorage.getItem('requestDetails')) ||
			this.state.requestDetails // JSON.parse( localStorage.getItem('requestDetails')) || this.state.requestDetails;
		...
		const statusPanel = () => {
			switch (path) {
					...
					return (
						<div className={'status-panel'}>
							...
							<button
								...
								onClick={this.handleDriverGoOnline}
							>
								GO ONLINE
							</button>
						</div>
						...
						<div className={'status-panel'}>
							...
							<div className={'trip-destination-container'}>
								<div className={'icon-box'}>
									...
									<h3>
										{' '}
										{`${requestDetails.rider.rating} `}stars
									</h3>
								</div>
								<div className={'icon-box'}>
								...
								<button
									className={'driver-item-accept-button'}
				...
				case 'waitingForConfirmation':
					return (
						<div className={'status-panel'}>
							<h1 className={`status-panel__header text-blue`}>
							...
							<div className={'trip-destination-container'}>
								...
								<div className={'icon-box'}>
									<img src={CarIcon} alt={'rider icon'} />
									<h3>{`${requestDetails.distance}`} miles</h3>
									{/*<h3>{`${requestDetails.duration}`} min.</h3>*/}
								</div>
								<div className={'trip-destination-left'}>
									<h1>Destination</h1>
									<h2>{requestDetails.endLocationAddress}</h2>
				...
				case 'confirmed':
					return (
						<div className={'status-panel'}>
							<h1 className={`status-panel__header inline`}>
								Trip Confirmed.{' '}
